  ...
  return (
    <section className='genre-list-main-container'>
      ...
      {genreStation.map(genre => {
        return <Link key={genre._id} className="text-decoration" to={`/genre/${genre.genre}`}><div style={{ backgroundColor: genre.color }} className="genre-preview"><span>{genre.genre}</span></div></Link>
      })}
    </section>
  )
}
